...
const Field = ({label, value, onChangeText, style}: any) => (
  <View style={style}>
...
const PickerField = ({label, value, onPress, style}: any) => (
  <View style={style}>
    <Text style={s.label}>{label}</Text>
...
export function AlibiAnalyzerScreen({navigation}: any) {
  ...
  const update = (key: keyof AlibiInput, value: any) =>
  ...
  if (result) {
    ...
    return (
      <Screen>
        ...
        <Card style={[s.resultCard, {borderColor: accent}] as any}>
    ...
    <Screen>
      ...
      <Section number="2" title="Travel Estimate">
        <View style={s.twoCol}>
          ...
          <PickerField label="Unit" value={input.unit} style={s.col} onPress={() => openPicker('Unit', ['km', 'mi'], input.unit, v => update('unit', v))} />
        </View>
      ...
      <Section number="3" title="Supporting Evidence">
        <PickerField label="Witness Reliability" value={input.reliability} onPress={() => openPicker('Witness Reliability', ['Low', 'Medium', 'High'], input.reliability, v => update('reliability', v))} />
        ...
        <PickerField label="Evidence Verification" value={input.verification} onPress={() => openPicker('Evidence Verification', ['Unverified', 'Partially Verified', 'Verified'], input.verification, v => update('verification', v))} />
...
function Header({navigation}: any) {
  return <View style={s.header}><View style={{flex: 1}}><Title>Alibi Analyzer</Title><Subtitle>Reconstruct the timeline and test whether the alibi holds.</Subtitle></View><Pressable style={s.iconButton} onPress={() => navigation.navigate('SavedAnalyses')}><Text style={s.iconText}>↶</Text></Pressable></View>;
}
...
function Action({label, onPress, primary}: any) {
  if (primary) return <Pressable style={{flex: 1}} onPress={onPress}><LinearGradient colors={[colors.pink, colors.coral]} start={{x: 0, y: 0}} end={{x: 1, y: 0}} style={s.action}><Text style={s.primaryActionText}>{label}</Text></LinearGradient></Pressable>;
  return <Pressable style={[s.action, s.secondaryAction]} onPress={onPress}><Text style={s.actionText}>{label}</Text></Pressable>;
}
function SelectionSheet({picker, close}: any) {
